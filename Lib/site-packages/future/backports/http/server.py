...
and CGIHTTPRequestHandler for CGI models.
...
This class implements GET and POST requests to cgi-bin models.
...
models cannot send other status codes such as 302 (redirect).
...
class CGIHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    GET and HEAD also support running CGI models.

    The POST command is *only* implemented for CGI models.
    ...
    def do_POST(self):
        ...
        This is only implemented for CGI models.
        ...
        if self.is_cgi():
            ...
            self.send_error(501, "Can only POST to CGI models")

    def send_head(self):
        """Version of send_head that support CGI models"""
        if self.is_cgi():
            return self.run_cgi()
        else:
